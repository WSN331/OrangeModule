...
public class ShitJSONHelper {
     ...
     * @return JSON文件
     */
    public static ShitJSONArray<?> fromList(Collection<?> list, final String selector) {
        return fromList(list, new JsonIterator() {
            @Override
            public ShitJSONObject<?> getJsonItem(Object object) {
                return fromObject(object, selector);
            }
        });
    }

    /**
     * 根据迭代器接口生成JSON的集合，迭代器声明了如何将元集合中的每个元素转变为JSON元素
     *
     * @param list     原java集合
     * @param iterator 迭代器
     * @return JSON集合
     */
    public static ShitJSONArray<?> fromList(Collection<?> list, JsonIterator iterator) {
        ShitJSONArray<?> array = driver.initJSONArray();
        if (list != null) {
            for (Object object : list) {
                array.add(iterator.getJsonItem(object));
            }
        }
        return array;
    }
     ...
     * @return
     */
    public static ShitJSONArray<?> fromList(String str) {
        ShitJSONArray<?> array = driver.initJSONArray();
        return array.fromString(str);
    }
    ...
    public interface JsonIterator {
         ...
         * @return json对象
         */
        ShitJSONObject<?> getJsonItem(Object object);
    }
